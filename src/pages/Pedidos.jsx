...
const TOP_LABELS = {
  '1000': 'Venda', '1007': 'Bonif.', '1008': 'Evento', '1009': 'Troca', '1010': 'Pre-ped.',
  'Venda': 'Venda', 'Troca': 'Troca', 'Bonif.': 'Bonif.',
};
const TOP_CORES = {
...
export default function Pedidos() {
  ...
  const regioes = [...new Set(pedidos.map(p => p.geo_zone || p.region).filter(Boolean))].sort();
  ...
  const filtrados = pedidos.filter(p => {
    ...
    if (filtroRegiao && (p.geo_zone || p.region || '') !== filtroRegiao) return false;
    if (filtroTop && String(p.order_type || '') !== filtroTop) return false;
    return true;
  }).slice(0, limite);

  const pesoTotal = filtrados.reduce((s, p) => s + (parseFloat(p.weight_kg) || 0), 0);
  const valorTotal = filtrados.reduce((s, p) => s + (parseFloat(p.total_value) || 0), 0);
  const comBoleto = filtrados.filter(p => p.payment_description && p.payment_description.toLowerCase().includes('boleto')).length;
  ...
  const importarCab = async (file) => {
    ...
    try {
      ...
      headers.forEach((h, i) => {
        ...
        else if (hl.includes('dt.') || hl.includes('dt neg')) idx.dtneg = i;
        else if (hl.includes('observa')) idx.obs = i;
        else if (hl.includes('tipo neg') && !hl.includes('descri')) idx.tiponeg = i;
        else if (hl.includes('descri') && hl.includes('negoci')) idx.tiponegdesc = i;
        else if (hl === 'nro. nota' || hl === 'nro nota') idx.nrnota = i;
      });
      setImportLog(prev => [...prev, '✅ Colunas mapeadas: ' + Object.keys(idx).join(', ')]);
      ...
      const { data: clientes } = await supabase
        .from('clients')
        .select('codparc,name,address,district,lat,lng,geo_zone,route,service_time')
        .in('codparc', codparcs);
      const cliMap = {};
      (clientes || []).forEach(c => { cliMap[c.codparc] = c; });
      setImportLog(prev => [...prev, `✅ ${Object.keys(cliMap).length} clientes encontrados`]);

      const pedidosParaImportar = rows.map((row, i) => {
        ...
        const paymentType = parseInt(row[idx.tiponeg]) || null;
        const paymentDesc = String(row[idx.tiponegdesc] || '').trim();
        const nrNota = String(row[idx.nrnota] || '').trim();
        const obs = String(row[idx.obs] || '').trim();
        ...
        return {
          ...
          address: cli.address ? `${cli.address}${cli.district ? ', ' + cli.district : ''}` : obs,
          ...
          order_type: topVal,
          invoice_number: nrNota || nunota,
          payment_type: paymentType,
          payment_description: paymentDesc || null,
          notes: obs || null,
      ...
      const comGps = pedidosParaImportar.filter(p => p.lat && p.lng).length;
      const comBoletoImp = pedidosParaImportar.filter(p => p.payment_description && p.payment_description.toLowerCase().includes('boleto')).length;
      setImportLog(prev => [...prev, `📍 ${comGps}/${pedidosParaImportar.length} com GPS`]);
      setImportLog(prev => [...prev, `🔔 ${comBoletoImp} com boleto detectado`]);

      const lotes = [];
      for (let i = 0; i < pedidosParaImportar.length; i += 50) lotes.push(pedidosParaImportar.slice(i, i + 50));
      ...
      setImportLog(prev => [...prev, `🎉 ${pedidosParaImportar.length} pedidos importados!`]);
      load();
    } catch (e) {
  ...
  const importarItens = async (file) => {
    ...
    try {
      ...
      headers.forEach((h, i) => {
        const hl = h.toLowerCase().trim();
        if (hl.includes('nro') && hl.includes('nico')) idx.nunota = i;
        else if (hl.includes('item') || hl.includes('produto')) idx.item = i;
        else if (hl.includes('quantidade')) idx.qty = i;
        else if (hl.includes('top')) idx.top = i;
        else if (hl === 'parceiro') idx.codparc = i;
        else if (hl.includes('peso')) idx.peso = i;
      });
      ...
      const todosItens = rows.filter(r => r[idx.nunota]).map((row, i) => {
        ...
        return {
          id: `oi-${nunota}-${i}`,
          external_id: nunota,
          invoice_number: nunota,
          codparc: parseInt(codparcStr.split(' - ')[0]) || null,
          item_name: itemName.trim(),
          qty: parseInt(row[idx.qty]) || 0,
          weight_unit: parseFloat(String(row[idx.peso] || '0').replace(',', '.')) || 0,
          top_app: String(row[idx.top] || '').split(' - ')[0] || '1000',
        };
      });
      ...
      const erros = resultados.filter(r => r.error);
      if (erros.length) setImportLog(prev => [...prev, `⚠️ ${erros.length} lotes com erro`]);
      setImportLog(prev => [...prev, `🎉 ${todosItens.length} itens importados!`]);
      load();
    } catch (e) {
      setImportLog(prev => [...prev, `❌ ${e.message}`]);
    } finally { setImportando(false); }
  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6,1fr)', gap: 10, marginBottom: 16 }}>
        ...
        <div className="card" style={{ textAlign: 'center' }}>
          <div style={{ fontSize: 11, color: '#90afd4', marginBottom: 2 }}>🔔 Com Boleto</div>
          <div style={{ fontSize: 16, fontWeight: 700, color: '#f59e0b' }}>{comBoleto}</div>
          <div style={{ fontSize: 10, color: '#90afd4' }}>R$ {valorTotal.toFixed(0)}</div>
        </div>
      </div>
      ...
      <div className="card" style={{ marginBottom: 16, display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
        ...
        <select className="form-control" style={{ width: 120 }} value={filtroTop} onChange={e => setFiltroTop(e.target.value)}>
          ...
          {Object.entries(TOP_LABELS).filter(([k]) => k.length <= 4).map(([k, v]) => <option key={k} value={k}>{v}</option>)}
        </select>
          ...
          <table>
            <thead>
              <tr>
                ...
                <th>Peso (kg)</th><th>Valor (R$)</th><th>TOP</th><th>Pagamento</th><th>GPS</th><th>Status</th><th>Ações</th>
              </tr>
            </thead>
            <tbody>
              {loading ? (
                ...
                const isBol = p.payment_description && p.payment_description.toLowerCase().includes('boleto');
                return (
                  <tr key={p.id} style={{ background: selecionados[p.id] ? 'rgba(232,82,26,.05)' : '' }}>
                    ...
                    <td style={{ fontFamily: 'monospace', color: '#64B4FF', fontSize: 12 }}>{p.external_id}</td>
                    ...
                    <td style={{ fontSize: 11, color: isBol ? '#f59e0b' : '#90afd4', fontWeight: isBol ? 700 : 400 }}>
                      {isBol ? '🔔 ' : ''}{p.payment_description || 'A Vista'}
                    </td>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.75)', zIndex: 3000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          <div style={{ background: '#0f2040', border: '1px solid #1e3a5c', borderRadius: 16, width: 520 }}>
            <div style={{ padding: '16px 24px', borderBottom: '1px solid #1e3a5c' }}>
              ...
              <div style={{ fontSize: 11, color: '#90afd4', marginTop: 2 }}>
                {modalImportTipo === 'cab' ? 'GPS, endereço e tipo de pagamento buscados automaticamente' : 'Vincula automaticamente aos pedidos pelo Nro. Único'}
              </div>
            </div>
              ...
              {!importando && importLog.length === 0 && (
                <label style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', border: '2px dashed #1e3a5c', borderRadius: 12, padding: 32, cursor: 'pointer', marginBottom: 16 }}>
                  ...
                  <input type="file" accept=".xlsx,.xls" style={{ display: 'none' }}
                    onChange={e => { if (e.target.files[0]) modalImportTipo === 'cab' ? importarCab(e.target.files[0]) : importarItens(e.target.files[0]); }} />
                </label>
              )}
