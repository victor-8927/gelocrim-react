...
export default function Pedidos() {
  ...
  const [modalImportTipo, setModalImportTipo] = useState(null); // 'cab' | 'itens' | 'ti'
  ...
  const importarCab = async (file) => {
    ...
    try {
      ...
      headers.forEach((h, i) => {
        ...
        else if (hl.includes('parceiro') && !hl.includes('nome') && !hl.includes('desc')) idx.codparc = i;
        else if (hl.includes('nome parceiro')) idx.nomeparc = i;
        else if (hl === 'peso') idx.peso = i;
        else if (hl.includes('vlr') || hl.includes('valor')) idx.vlrnota = i;
        else if (hl.includes('tipo opera')) idx.top = i;
        else if (hl.includes('dt.') || hl.includes('data')) idx.dtneg = i;
        else if (hl.includes('observa')) idx.obs = i;
        else if (hl.includes('rota')) idx.rota = i;
      });

      setImportLog(prev => [...prev, `✅ Colunas mapeadas: NUNOTA, CODPARC, NOMEPARC, PESO, VLRNOTA`]);

      // Buscar todos os codparcs únicos no Supabase
      const codparcs = [...new Set(rows.map(r => parseInt(r[idx.codparc]) || 0).filter(c => c > 0))];
      setImportLog(prev => [...prev, `🔍 Buscando ${codparcs.length} clientes no banco...`]);
      ...
      setImportLog(prev => [...prev, `✅ ${Object.keys(cliMap).length} clientes encontrados no banco`]);
      ...
      const pedidos = rows.map((row, i) => {
        ...
        const peso = parseFloat(String(row[idx.peso] || '0').replace(',', '.')) || 0;
        const valor = parseFloat(String(row[idx.vlrnota] || '0').replace(',', '.')) || 0;
        const top = String(row[idx.top] || '1000').trim();
        const topLabel = top === '1000' ? 'Venda' : top === '1010' ? 'Troca' : top === '1020' ? 'Bonif.' : top === '1030' ? 'Pre-ped.' : top;
        const dataStr = row[idx.dtneg] ? String(row[idx.dtneg]).slice(0, 10) : new Date().toISOString().slice(0, 10);

        return {
          id: `ord-${nunota}-${Date.now()}`,
          external_id: nunota,
          codparc: codparc,
      ...
      setImportLog(prev => [...prev, `📍 ${comGps}/${pedidos.length} pedidos com GPS`]);
      ...
      setImportLog(prev => [...prev, `🎉 ${inseridos} pedidos importados com dados completos!`]);
      load();
    } catch (e) {
  ...
  const importarItens = async (file) => {
    ...
    try {
      ...
      headers.forEach((h, i) => {
        ...
        else if (hl.includes('quantidade') || hl === 'q') idx.qty = i;
        else if (hl.includes('top')) idx.top = i;
        else if (hl.includes('parceiro')) idx.codparc = i;
      });
      ...
      rows.forEach(row => {
        const nunota = String(row[idx.nunota] || '').trim();
        if (!nunota) return;
        if (!itensPorNota[nunota]) itensPorNota[nunota] = [];
        ...
        const codparc = parseInt(codparcStr.split(' - ')[0]) || null;
        itensPorNota[nunota].push({
          id: `oi-${nunota}-${Date.now()}-${Math.random()}`,
      ...
      setImportLog(prev => [...prev, `✅ ${Object.keys(itensPorNota).length} notas com itens`]);

      // Inserir itens
      const todosItens = Object.values(itensPorNota).flat();
      const lotes = [];
      ...
      for (const lote of lotes) {
        ...
        if (error) setImportLog(prev => [...prev, `⚠️ ${error.message}`]);
        else { inseridos += lote.length; setImportLog(prev => [...prev, `✅ ${inseridos}/${todosItens.length} itens`]); }
      }
      setImportLog(prev => [...prev, `🎉 ${inseridos} itens importados e vinculados!`]);
      load();
    } catch (e) {
      setImportLog(prev => [...prev, `❌ Erro: ${e.message}`]);
    } finally { setImportando(false); }
