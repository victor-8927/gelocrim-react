...
// ── IMPORTAÇÃO XLSX ──────────────────────────────────────────────────────────
function getNorm(row, ...keys) {
  const r = {};
  Object.keys(row).forEach(k => {
    const n = k.trim().toUpperCase().replace(/\./g, '').replace(/\s+/g, ' ');
    r[n] = row[k];
  });
  for (const k of keys) {
    const tries = [
    ...
    for (const t of tries) {
      const v = r[t];
      if (v !== undefined && v !== null && String(v).trim() !== '' && String(v).trim() !== 'None' && String(v).trim() !== 'undefined') {
...
function parseCodNome(val) {
  ...
  if (m) return { codparc: parseInt(m[1]), nome: m[2].trim() };
  if (/^\d+$/.test(s)) return { codparc: parseInt(s), nome: '' };
  return { codparc: null, nome: s };
}

function mapearCab(row) {
  const oc = getNorm(row, 'ORDEM DE CARGA', 'ORDEMCARGA');
  if (oc !== '' && oc !== 'NONE' && parseInt(oc) !== 0) return null;
  const nunota = getNorm(row, 'NRO ÚNICO', 'NRO UNICO', 'NUMERO ÚNICO', 'NUMERO UNICO', 'NUNOTA');
  if (!nunota) return null;
  const nf      = getNorm(row, 'NRO NOTA', 'NUMNOTA', 'NUMERO DOCUMENTO') || nunota;
  const codparc = parseInt(getNorm(row, 'PARCEIRO', 'CODPARC') || '0') || null;
  const nome    = getNorm(row, 'NOME PARCEIRO (PARCEIRO)', 'NOMEPARC', 'NOME PARCEIRO', 'NOME PARCEIROS');
  const data    = getNorm(row, 'DT NEG', 'DTNEG', 'DATA');
  const top     = getNorm(row, 'TIPO OPERAÇÃO', 'TIPO OPERACAO', 'CODTIPOPER', 'TOP') || '1000';
  const peso    = getNorm(row, 'PESO', 'PESOBRUT');
  const vlr     = getNorm(row, 'VLR NOTA', 'VLRNOTA', 'VALOR NOTA', 'VALOR');
  const pagto   = getNorm(row, 'DESCRIÇÃO (TIPO DE NEGOCIAÇÃO)', 'DESCRICAO (TIPO DE NEGOCIACAO)', 'PAGAMENTO') || 'A Vista';
  const geo     = getNorm(row, 'DESCRIÇÃO (ROTA)', 'DESCRICAO (ROTA)', 'REGIAO', 'REGIÃO');
  ...
  if (data) {
    try {
      ...
      else if (data.includes('T')) dataFmt = data.slice(0, 10);
    } catch(e) {}
  }
  return {
    ...
    codparc: codparc,
    recipient_name: nome || (codparc ? 'CODPARC ' + codparc : '—'),
...
function mapearItem(row) {
  const oc = getNorm(row, 'ORDEM DE CARGA', 'ORDEMCARGA');
  if (oc !== '' && oc !== 'NONE' && parseInt(oc) !== 0) return null;
  const nunota  = getNorm(row, 'NRO ÚNICO', 'NRO UNICO', 'NUMERO ÚNICO', 'NUNOTA');
  const itemRaw = getNorm(row, 'ITEM');
  if (!nunota || !itemRaw) return null;
  const parts   = itemRaw.split(/\s*[-–]\s*/);
  const codprod = parts[0].trim();
  const descr   = parts.slice(1).join(' ').trim();
  const nf      = getNorm(row, 'NRO DOC', 'NUMNOTA', 'NUMERO DOCUMENTO') || nunota;
  const { codparc } = parseCodNome(getNorm(row, 'PARCEIRO', 'CODPARC'));
  const qty     = getNorm(row, 'QUANTIDADE');
  const topRaw  = getNorm(row, 'TOP');
  const topM    = topRaw ? topRaw.match(/^(\d+)/) : null;
  ...
  return {
    ...
    top_app:        topM ? topM[1] : (topRaw || '').replace(/\D/g, '') || '1000',
  };
}
  ...
  return new Promise((resolve) => {
    ...
    reader.onload = async (e) => {
      try {
        ...
        const mapear = tipo === 'cab' ? mapearCab : mapearItem;
        const tabela = tipo === 'cab' ? 'orders' : 'order_items';
        const validos = rows.map(mapear).filter(Boolean);
        if (validos.length === 0) { setMsg('⚠️ Nenhum registro válido encontrado.'); resolve(0); return; }
        ...
        if (tipo === 'cab') {
          ...
          if (ids.length > 0) await supabase.from('order_items').delete().in('order_id', ids);
        }
        setMsg('⏳ Importando ' + validos.length + ' registros...');
        const LOTE = 50; let ok = 0; let err = 0;
        ...
        setMsg(msg);
        resolve(ok);
      } catch (e) { setMsg('❌ Erro: ' + e.message); resolve(0); }
    };
...
function ModalPedido({ pedido, onFechar }) {
  ...
  React.useEffect(() => {
    ...
    supabase.from('order_items').select('item_type, item_name, qty, weight_unit, top_app').eq('order_id', orderId)
      ...
      <div style={{ background:'#0f2040', border:'1px solid #1e3a5c', borderRadius:16, width:520, maxHeight:'90vh', overflowY:'auto' }}>
        ...
        <div style={{ padding:20, display:'flex', flexDirection:'column', gap:14 }}>
          ...
          <div>
            ...
            {loadingItens ? <div style={{ color:'#90afd4', fontSize:12 }}>Carregando...</div>
            : itens.length === 0 ? <div style={{ color:'#90afd4', fontSize:12, padding:'8px 12px', background:'#0a1628', borderRadius:8, border:'1px solid #1e3a5c' }}>Sem itens — importe os Itens do Sankhya</div>
            : (
                  ...
                  <tbody>
                    {itens.map((item, i) => (
                      <tr key={i} style={{ borderTop:'1px solid #1e3a5c' }}>
                        ...
                        <td style={{ padding:'5px 8px', fontSize:12, fontWeight:700, color:topCor, textAlign:'center' }}>{item.qty}</td>
                        <td style={{ padding:'5px 8px', fontSize:11, color:'#90afd4', textAlign:'center' }}>{item.weight_unit} kg</td>
...
export default function Pedidos() {
  ...
  const regioes = [...new Set(pedidos.map(p => p.region || p.geo_zone || '').filter(Boolean))].sort();

  const filtrados = pedidos.filter(p => {
    if (busca) {
      const b = busca.toLowerCase();
      if (!(p.recipient_name||'').toLowerCase().includes(b) &&
          !String(p.external_id||'').includes(b) &&
          !String(p.codparc||'').includes(b) &&
          !(p.address||'').toLowerCase().includes(b)) return false;
    }
    if (status && p.status !== status) return false;
    if (filtroRegiao && (p.region || p.geo_zone || '') !== filtroRegiao) return false;
    if (filtroTop && String(p.order_type || '') !== filtroTop) return false;
    return true;
  }).slice(0, limite);
    ...
    <div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(6,1fr)', gap:10, marginBottom:16 }}>
        ...
        <div className="card" style={{ textAlign:'center' }}>
          ...
          <div style={{ fontSize:10, color:'#90afd4' }}>Cap: —</div>
        </div>
      </div>
      ...
      {/* Pendente por TOP */}
      {(() => {
        const pend = pedidos.filter(p => p.status === 'pending');
        const tops = ['1000','1009','1007','1008','1010'];
        const res  = {};
        pend.forEach(p => {
          const t = p.order_type || '1000';
          if (!res[t]) res[t] = { peso:0, qtd:0 };
          res[t].peso += parseFloat(p.weight_kg) || 0;
          res[t].qtd += 1;
        });
        const ativos = tops.filter(t => res[t]);
          ...
          <div style={{ background:'#0a1628', border:'1px solid rgba(100,180,255,0.2)', borderRadius:10, padding:'8px 16px', marginBottom:12 }}>
            ...
            {ativos.map(t => (
              <span key={t} style={{ marginRight:16, fontSize:12 }}>
                ...
                <span style={{ color:'#90afd4' }}> {res[t].qtd} ped · {res[t].peso.toFixed(0)}kg</span>
              </span>
            ))}
          </div>
        );
              ...
              {loading ? (
                ...
                return (
                  <tr key={p.id} style={{ background:selecionados[p.id] ? 'rgba(232,82,26,.05)' : '' }}>
                    ...
                    <td>
                      <span onClick={() => setModalGps(p)} style={{ cursor:'pointer', color:p.lat && p.lng ? '#10b981' : '#ef4444', fontSize:12, fontWeight:700, textDecoration:'underline dotted' }} title="Clique para geolocalizar">
                        {p.lat && p.lng ? '✓ GPS' : '📍 —'}
                      </span>
                    </td>
