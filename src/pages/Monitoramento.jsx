  ...
  useEffect(() => {
    ...
    gps.forEach(g => {
      ...
      const mk = new window.google.maps.Marker({
        ...
        map: mapObj.current, title: g.driver_name || 'Motorista',
        icon: { path: window.google.maps.SymbolPath.FORWARD_CLOSED_ARROW, scale: 6, fillColor: '#e8521a', fillOpacity: 1, strokeColor: '#fff', strokeWeight: 2 }
      });
    ...
    <div style={{ display: 'flex', gap: 16, height: 'calc(100vh - 56px - 48px)', overflow: 'hidden' }}>
      ...
      <div style={{ width: 320, flexShrink: 0, display: 'flex', flexDirection: 'column', gap: 10, overflow: 'hidden' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5,1fr)', gap: 6 }}>
          {[
            ...
            { emoji: '📊', label: 'Progresso', value: `${progresso}%`, cor: '#f59e0b' },
          ].map(k => (
            ...
            return (
              <div key={r.id} onClick={() => setRotaSel(sel ? null : r.id)}
                ...
                <div style={{ fontSize: 11, color: '#90afd4', marginBottom: 6 }}>
                  {r.vehicle_name || r.vehicle?.plate || '—'} · {r.driver_name || r.driver?.name || '—'}
                </div>
        ...
        {rotaAtual && (
          <div style={{ borderTop: '1px solid #1e3a5c', paddingTop: 10, maxHeight: 260, overflowY: 'auto' }}>
            ...
            {(rotaAtual.stops || rotaAtual.orders || []).map((s, i) => {
              const ok = s.status === 'delivered' || s.status === 'completed';
              const recusou = s.status === 'refused' || s.status === 'failed';
              return (
                <div key={i} style={{ display: 'flex', gap: 8, padding: '5px 0', borderBottom: '1px solid rgba(30,58,92,.5)' }}>
                  <div style={{ width: 18, height: 18, borderRadius: '50%', background: ok ? '#10b981' : recusou ? '#ef4444' : '#1e3a5c', color: '#fff', fontSize: 9, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>{i + 1}</div>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 11, fontWeight: 600, color: ok ? '#10b981' : recusou ? '#ef4444' : '#e8f0fe' }}>
                      {ok ? '✅' : recusou ? '❌' : '⏳'} {s.recipient_name || s.name || '—'}
                    </div>
                    <div style={{ fontSize: 10, color: '#90afd4' }}>
                      {s.arrival_time || s.eta || '—'}{s.notes ? ` · ${s.notes}` : ''}
                    </div>
                  </div>
                </div>
              );
            })}
            {!(rotaAtual.stops || rotaAtual.orders || []).length && (
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 8 }}>
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          ...
          {gps.length === 0 && <span style={{ fontSize: 11, color: '#90afd4', marginLeft: 'auto' }}>Nenhum GPS ativo</span>}
        </div>
