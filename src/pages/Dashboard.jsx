...
import { getOrdersSummary, getRoutesToday, getFleetSummary, getDriversSummary } from '../services/supabase';
...
export default function Dashboard() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const [o, r, f, e] = await Promise.all([
        getOrdersSummary().catch(() => ({})),
        getRoutesToday().catch(() => []),
        getFleetSummary().catch(() => ({})),
        getDriversSummary().catch(() => ({})),
      ]);
      setOrders(o || {});
  ...
  // KPIs
  const pendentes = orders.pendentes || 0;
  const emRota = orders.roteirizados || 0;
  const entregues = orders.entregues || 0;
  const falhas = orders.falhas || 0;
  const devolvidos = orders.devolvidos || 0;
  const reprogramados = orders.reprogramados || 0;
  ...
  const kmHoje = rotas.reduce((s, r) => s + parseFloat(r.total_distance_km || 0), 0);
  const veicAtivos = frota.veiculos_ativos || 0;
  const motorEmCampo = equipe.motoristas_ativos || 0;

  // Financeiro
  const fatTotal = orders.faturamento_entregues || 0;
  const kmPerLiter = rotas.length > 0 ? rotas.reduce((s,r) => s + parseFloat(r.km_per_liter||3), 0) / rotas.length : 3;
  const fuelPrice = rotas.length > 0 ? rotas.reduce((s,r) => s + parseFloat(r.fuel_price||7.59), 0) / rotas.length : 7.59;
  const custoDiesel = kmHoje / kmPerLiter * fuelPrice;
  const custoEquipe = rotas.reduce((s,r) => s + parseFloat(r.driver_cost||0), 0);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div className="card" style={{ padding: '14px 20px' }}>
        ...
        <div style={{ fontSize: 12, color: '#90afd4' }}>{entregues} entregues · {total} total · {orders.peso_total ? `${orders.peso_total.toFixed(0)} kg` : '0 kg'}</div>
        {margem > 0 && margem < 10 && (
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 16 }}>
        ...
        <div className="card">
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {[
              ...
              { emoji: '❌', label: 'Com Falha', value: falhas, sub: 'não entregues', cor: '#ef4444' },
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div className="card">
            ...
            {[
              ...
              { emoji: '🚐', label: 'Frotas Ativas', value: veicAtivos, sub: `${motorEmCampo} motoristas`, cor: '#10b981' },
          ...
          <div className="card">
            ...
            {[
              { emoji: '💵', label: 'Faturamento', value: fatTotal > 0 ? `R$ ${(fatTotal/1000).toFixed(1)}k` : 'R$ 0,0k', sub: 'entregues hoje', cor: '#10b981' },
              { emoji: '📊', label: 'Margem Op.', value: `${margem.toFixed(1)}%`, sub: 'rentabilidade', cor: margem >= 20 ? '#10b981' : margem >= 10 ? '#f59e0b' : '#ef4444' },
              { emoji: '⛽', label: 'Custo Diesel', value: `R$ ${custoDiesel.toFixed(0)}`, sub: `${(kmHoje/kmPerLiter).toFixed(0)}L · R$${fuelPrice.toFixed(2)}/L`, cor: '#f59e0b' },
              { emoji: '💲', label: 'Custo/Entrega', value: custoEntrega > 0 ? `R$ ${custoEntrega.toFixed(0)}` : 'R$ —', sub: 'eficiência', cor: '#64B4FF' },
            ].map(k => (
              <div key={k.label} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '6px 0', borderBottom: '1px solid rgba(30,58,92,.5)' }}>
                ...
                <div style={{ fontSize: 13, fontWeight: 700, color: k.cor }}>{k.value}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div className="card">
            <div style={{ fontSize: 11, fontWeight: 700, color: '#f59e0b', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: 10 }}>⚡ ALERTAS</div>
            {falhas > 0 && <div style={{ fontSize: 12, color: '#ef4444', padding: '6px 10px', background: 'rgba(239,68,68,.1)', borderRadius: 6, marginBottom: 6 }}>❌ {falhas} entrega(s) com falha</div>}
            {devolvidos > 0 && <div style={{ fontSize: 12, color: '#a78bfa', padding: '6px 10px', background: 'rgba(167,139,250,.1)', borderRadius: 6, marginBottom: 6 }}>↩️ {devolvidos} devolução(ões)</div>}
            {falhas === 0 && devolvidos === 0 && <div style={{ fontSize: 12, color: '#10b981' }}>✅ Nenhum alerta no momento</div>}
          </div>

          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            <div className="card" style={{ textAlign: 'center', padding: 12 }}>
              <div style={{ fontSize: 11, color: '#90afd4', marginBottom: 4 }}>⚖️ Peso Pendente</div>
              <div style={{ fontSize: 16, fontWeight: 700, color: '#f59e0b' }}>{orders.peso_pendente ? `${(orders.peso_pendente/1000).toFixed(1)}t` : '—'}</div>
            </div>
            <div className="card" style={{ textAlign: 'center', padding: 12 }}>
              <div style={{ fontSize: 11, color: '#90afd4', marginBottom: 4 }}>🚛 Frota Total</div>
              <div style={{ fontSize: 16, fontWeight: 700, color: '#64B4FF' }}>{frota.total_veiculos || '—'}</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
        ...
        <table>
          <thead>
            <tr>
              <th>Nº Viagem</th><th>Veículo</th><th>Motorista</th><th>Paradas</th><th>Início</th><th>Status</th>
            </tr>
          </thead>
          <tbody>
            {loading ? (
              <tr><td colSpan={6} style={{ textAlign: 'center', color: '#90afd4', padding: 20 }}>Carregando...</td></tr>
            ) : rotas.length === 0 ? (
              <tr><td colSpan={6} style={{ textAlign: 'center', color: '#90afd4', padding: 20 }}>Nenhuma rota hoje</td></tr>
            ) : rotas.slice(0, 8).map(r => {
              const done = r.delivered_stops || 0;
              const tot = r.total_stops || 0;
              const pct = tot > 0 ? Math.round(done / tot * 100) : 0;
              return (
                <tr key={r.id}>
                  <td style={{ fontWeight: 600, fontSize: 12, color: '#64B4FF' }}>{r.trip_number || '—'}</td>
                  <td style={{ fontSize: 12 }}>{r.vehicle_name || '—'}</td>
                  <td style={{ fontSize: 12, color: '#90afd4' }}>{r.driver_name || '—'}</td>
                  <td style={{ fontSize: 12 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                      <div style={{ width: 50, height: 4, background: '#1e3a5c', borderRadius: 2, overflow: 'hidden' }}>
